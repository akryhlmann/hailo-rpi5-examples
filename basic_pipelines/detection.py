...
def app_callback(pad, info, user_data):
    ...
    # Midten af billedet (horisontal linje)
    line_y = int(height / 2)
...
for detection in detections:
    ...
    bbox = detection.get_bbox()  # [x1, y1, x2, y2]
    ...
    if label == "person":
        ...
        if len(track) == 1:
            ...
            # Beregn midtpunkt af bbox
            x_center = int((bbox[0] + bbox[2]) / 2)
            y_center = int((bbox[1] + bbox[3]) / 2)

            prev_y = user_data.track_positions.get(track_id, None)
            user_data.track_positions[track_id] = y_center  # Opdatér position
